...
Sethos: {
  ...
  ataques: {
    normal: {
      descricao: "Ataques rápidos de espada, causando dano Cryo.",
      executar: (status, atributos) => atributos.atk * 1.2
    },
    elemental: {
      descricao: "Golpe rápido Cryo que congela inimigos por 2s.",
      executar: (status, atributos, equipe = [], inimigos = []) => {
        inimigos.forEach(i => i.congelado = true);
        return atributos.atk * 1.5;
      }
    },
    ult: {
      descricao: "Invoca uma tempestade de gelo, causando dano em área e aplica Cryo.",
      executar: (status, atributos, equipe = [], inimigos = []) => {
        inimigos.forEach(i => i.hpAtual -= atributos.atk * 3.0);
        return true;
      }
    }
  },
  constelacoes: {
    c1: { descricao: "Aumenta dano do ataque normal em 15%", aplicar: (status) => { status.danoTotal = (status.danoTotal || 0) + 15; } },
    c2: { descricao: "Reduz recarga da elemental em 20%", aplicar: (status) => { status.cdHabilidadeElemental = (status.cdHabilidadeElemental || 100) - 20; } },
    c3: { descricao: "Ataque normal causa +20% dano Cryo", aplicar: (status) => { status.bonusDanoCryo = (status.bonusDanoCryo || 0) + 20; } },
    c4: { descricao: "Aumenta taxa crítica em 15%", aplicar: (status) => { status.taxaCritica = (status.taxaCritica || 0) + 15; } },
    c5: { descricao: "Reduz cooldown do ult em 25%", aplicar: (status) => { status.cdUlt = (status.cdUlt || 100) - 25; } },
    c6: { descricao: "Ataque supremo aumenta dano Cryo da equipe em 20%", aplicarUlt: (status) => { status.bonusDanoCryoEquipe = (status.bonusDanoCryoEquipe || 0) + 20; } }
  }
},
...
KamisatoAyato: {
  ...
  ataques: {
    normal: {
      descricao: "Ataques com espada Hydro rápidos.",
      executar: (status, atributos) => atributos.atk * 1.15
    },
    elemental: {
      descricao: "Golpe de água que causa dano Hydro em linha reta.",
      executar: (status, atributos, equipe = [], inimigos = []) => {
        inimigos.forEach(i => i.hpAtual -= atributos.atk * 1.8);
        return true;
      }
    },
    ult: {
      descricao: "Chama uma cascata Hydro que aumenta velocidade de ataque da equipe.",
      executar: (status, atributos, equipe = [], inimigos = []) => {
        equipe.forEach(p => p.status.velocidadeAtaque = (p.status.velocidadeAtaque || 100) + 20);
        inimigos.forEach(i => i.hpAtual -= atributos.atk * 2.5);
        return true;
      }
    }
  },
  constelacoes: {
    c1: { descricao: "Aumenta ataque normal em 15%", aplicar: (status) => { status.danoTotal = (status.danoTotal || 0) + 15; } },
    c2: { descricao: "Reduz recarga da elemental em 15%", aplicar: (status) => { status.cdHabilidadeElemental = (status.cdHabilidadeElemental || 100) - 15; } },
    c3: { descricao: "Aumenta dano da elemental em 20%", aplicar: (status) => { status.bonusDanoHydro = (status.bonusDanoHydro || 0) + 20; } },
    c4: { descricao: "Aumenta taxa crítica em 10%", aplicar: (status) => { status.taxaCritica = (status.taxaCritica || 0) + 10; } },
    c5: { descricao: "Reduz cooldown do ult em 20%", aplicar: (status) => { status.cdUlt = (status.cdUlt || 100) - 20; } },
    c6: { descricao: "Ult aumenta dano Hydro da equipe em 25%", aplicarUlt: (status) => { status.bonusDanoHydroEquipe = (status.bonusDanoHydroEquipe || 0) + 25; } }
  }
},
...
Yoimiya: {
  ...
  ataques: {
    normal: {
      descricao: "Ataques com arco, dano Pyro moderado.",
      executar: (status, atributos) => atributos.atk * 1.2
    },
    elemental: {
      descricao: "Dispara flechas de fogo, causando dano em área e aplica Pyro.",
      executar: (status, atributos, equipe = [], inimigos = []) => {
        inimigos.forEach(i => i.hpAtual -= atributos.atk * 1.8);
        return true;
      }
    ...
    ult: {
      descricao: "Show de fogos de artifício, dano massivo Pyro e aumenta ataque aliado.",
      executar: (status, atributos, equipe = [], inimigos = []) => {
        inimigos.forEach(i => i.hpAtual -= atributos.atk * 3.0);
        equipe.forEach(p => p.status.atk = (p.status.atk || atributos.atk) + atributos.atk * 0.2);
        return true;
      }
    }
  },
  constelacoes: {
    c1: { descricao: "Aumenta ataque normal em 20%", aplicar: (status) => { status.danoTotal = (status.danoTotal || 0) + 20; } },
    c2: { descricao: "Reduz recarga da elemental em 15%", aplicar: (status) => { status.cdHabilidadeElemental = (status.cdHabilidadeElemental || 100) - 15; } },
    c3: { descricao: "Ataque supremo aumenta dano Pyro em 20%", aplicar: (status) => { status.bonusDanoPyro = (status.bonusDanoPyro || 0) + 20; } },
    c4: { descricao: "Aumenta taxa crítica em 10%", aplicar: (status) => { status.taxaCritica = (status.taxaCritica || 0) + 10; } },
    c5: { descricao: "Reduz cooldown do ult em 20%", aplicar: (status) => { status.cdUlt = (status.cdUlt || 100) - 20; } },
    c6: { descricao: "Ataques normais geram dano Pyro extra em 25%", aplicarUlt: (status) => { status.bonusDanoPyroExtra = (status.bonusDanoPyroExtra || 0) + 25; } }
  }
},
...
Chiori: {
  ...
  ataques: {
    normal: {
      descricao: "Ataques leves com espada, dano Geo moderado.",
      executar: (status, atributos) => atributos.atk * 1.1
    },
    elemental: {
      descricao: "Cria barreira Geo que protege aliados próximos e causa dano aos inimigos.",
      executar: (status, atributos, equipe = [], inimigos = []) => {
        equipe.forEach(p => p.status.escudoBonus = (p.status.escudoBonus || 0) + 20);
        inimigos.forEach(i => i.hpAtual -= atributos.atk * 1.5);
        return true;
      }
    },
    ult: {
      descricao: "Invoca pilar Geo, causando dano massivo e fortalecendo barreiras.",
      executar: (status, atributos, equipe = [], inimigos = []) => {
        inimigos.forEach(i => i.hpAtual -= atributos.atk * 3.0);
        equipe.forEach(p => p.status.escudoBonus = (p.status.escudoBonus || 0) + 30);
        return true;
      }
    }
  },
  constelacoes: {
    c1: { descricao: "Aumenta ataque normal em 15%", aplicar: (status) => { status.danoTotal = (status.danoTotal || 0) + 15; } },
    c2: { descricao: "Reduz cooldown da elemental em 20%", aplicar: (status) => { status.cdHabilidadeElemental = (status.cdHabilidadeElemental || 100) - 20; } },
    c3: { descricao: "Aumenta dano da ult em 20%", aplicar: (status) => { status.bonusDanoUlt = (status.bonusDanoUlt || 0) + 20; } },
    c4: { descricao: "Aumenta taxa crítica em 10%", aplicar: (status) => { status.taxaCritica = (status.taxaCritica || 0) + 10; } },
    c5: { descricao: "Reduz cooldown do ult em 15%", aplicar: (status) => { status.cdUlt = (status.cdUlt || 100) - 15; } },
    c6: { descricao: "Ult fortalece escudos aliados em 30%", aplicarUlt: (status) => { status.escudoBonus = (status.escudoBonus || 0) + 30; } }
  }
},
...
AratakiItto: {
  ...
  ataques: {
    normal: {
      descricao: "Ataques pesados com maça, causando dano Geo.",
      executar: (status, atributos) => atributos.atk * 1.3
    },
    elemental: {
      descricao: "Salto explosivo causando dano Geo em área.",
      executar: (status, atributos, equipe = [], inimigos = []) => {
        inimigos.forEach(i => i.hpAtual -= atributos.atk * 1.7);
        return true;
      }
    },
    ult: {
      descricao: "Invoca Oni, causando dano massivo e aumenta ataque próprio.",
      executar: (status, atributos, equipe = [], inimigos = []) => {
        inimigos.forEach(i => i.hpAtual -= atributos.atk * 3.5);
        status.atk = (status.atk || atributos.atk) + atributos.atk * 0.25;
        return true;
      }
    }
  },
  constelacoes: {
    c1: { descricao: "Aumenta ataque normal em 20%", aplicar: (status) => { status.danoTotal = (status.danoTotal || 0) + 20; } },
    c2: { descricao: "Reduz cooldown da elemental em 15%", aplicar: (status) => { status.cdHabilidadeElemental = (status.cdHabilidadeElemental || 100) - 15; } },
    c3: { descricao: "Aumenta dano da ult em 25%", aplicar: (status) => { status.bonusDanoUlt = (status.bonusDanoUlt || 0) + 25; } },
    c4: { descricao: "Aumenta taxa crítica em 10%", aplicar: (status) => { status.taxaCritica = (status.taxaCritica || 0) + 10; } },
    c5: { descricao: "Reduz cooldown do ult em 20%", aplicar: (status) => { status.cdUlt = (status.cdUlt || 100) - 20; } },
    c6: { descricao: "Ataques normais causam dano Geo extra em 30%", aplicarUlt: (status) => { status.bonusDanoGeoExtra = (status.bonusDanoGeoExtra || 0) + 30; } }
  }
},
...
Clorinde: {
  ...
  ataques: {
    normal: {
      descricao: "Ataques com lâmina leve, dano Dendro moderado.",
      executar: (status, atributos) => atributos.atk * 1.1
    },
    elemental: {
      descricao: "Causa dano Dendro e aplica veneno que reduz ataque inimigo.",
      executar: (status, atributos, equipe = [], inimigos = []) => {
        inimigos.forEach(i => { i.hpAtual -= atributos.atk * 1.5; i.atk = (i.atk || 100) - 10; });
        return true;
      }
    },
    ult: {
      descricao: "Invoca floresta Dendro, causando dano massivo e curando aliados próximos.",
      executar: (status, atributos, equipe = [], inimigos = []) => {
        inimigos.forEach(i => i.hpAtual -= atributos.atk * 3.0);
        equipe.forEach(p => p.status.hpAtual = Math.min(p.atributos.hpMax, (p.status.hpAtual || p.atributos.hpMax) + atributos.hpMax * 0.2));
        return true;
      }
    }
  },
  constelacoes: {
    c1: { descricao: "Aumenta ataque normal em 15%", aplicar: (status) => { status.danoTotal = (status.danoTotal || 0) + 15; } },
    c2: { descricao: "Reduz cooldown da elemental em 15%", aplicar: (status) => { status.cdHabilidadeElemental = (status.cdHabilidadeElemental || 100) - 15; } },
    c3: { descricao: "Aumenta dano da elemental em 20%", aplicar: (status) => { status.bonusDanoDendro = (status.bonusDanoDendro || 0) + 20; } },
    c4: { descricao: "Aumenta taxa crítica em 10%", aplicar: (status) => { status.taxaCritica = (status.taxaCritica || 0) + 10; } },
    c5: { descricao: "Reduz cooldown do ult em 20%", aplicar: (status) => { status.cdUlt = (status.cdUlt || 100) - 20; } },
    c6: { descricao: "Ult aumenta cura Dendro da equipe em 25%", aplicarUlt: (status) => { status.bonusCuraDendroEquipe = (status.bonusCuraDendroEquipe || 0) + 25; } }
  }
}

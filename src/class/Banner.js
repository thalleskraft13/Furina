...
class Banner {
  constructor(client) {
    ...
    // Mapeamento de elementos dos personagens
  ...
  async push(pity, userId, value, bannerId = "1") {
    ...
    for (let i = 0; i < value; i++) {
      ...
      let chance5 = 0.6;
      if (pity.five >= maxPity - 15) {
        chance5 += (pity.five - (maxPity - 15) + 1) * 6;
      }
    ...
    // Atualizar personagens obtidos
    if (!Array.isArray(userdb.personagens)) userdb.personagens = [];
    ...
    resultado.forEach(({ type, nome }) => {
      ...
      if (index === -1) {
        userdb.personagens.push({
          nome, c: 0, level: 0, ascensao: 0, xp: 0, elemento,
          atributos: {
            hp: 1000, atk: 100, def: 50, recargaEnergia: 100,
            taxaCritica: 5, danoCritico: 50,
            bonusPyro: 0, bonusHydro: 0, bonusElectro: 0,
            bonusCryo: 0, bonusAnemo: 0, bonusGeo: 0,
            bonusDendro: 0, bonusFisico: 0
          },
          talentos: {
            ataqueNormal: 1, ataqueCarga: 1,
            habilidadeElemental: 1, supremo: 1
          }
        });
      } else {
    ...
    if (value >= 10) await desbloquear(1);
    if (value >= 50) await desbloquear(4);
    if (resultado.filter(i => i.raridade === 5 && i.type === "Personagem").length >= 5) await desbloquear(2);
    if (pity.five === 0 && value === 1) await desbloquear(3);
    if (value === 1 && resultado.some(i => i.raridade === 5)) await desbloquear(6);
    if (!conquistasUsuario.includes(7) && resultado.some(i => i.raridade === 4 && i.type === "Personagem")) {
      const jaTem = userdb.personagens.length > 1;
      if (!jaTem) await desbloquear(7);
    }

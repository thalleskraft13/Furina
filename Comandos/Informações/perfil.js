    ...
    if (!userdb) {
      ...
      await interaction.editReply({
        ...
        components: [new ContainerBuilder()
                ...
                new SectionBuilder()
                    .setThumbnailAccessory(
                        new ThumbnailBuilder()
                            .setURL("https://i.imgur.com/bNp2LFp.png")
                    )
            ...
            .addTextDisplayComponents(
                new TextDisplayBuilder().setContent(`Pity: ${userdb.gacha.pity.five}\nPrimogemas: ${userdb.primogemas}\nMora: ${userdb.mora}\n`),
            )
            .addSeparatorComponents(
                new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true),
            )
            .addSectionComponents(
                new SectionBuilder()
                    .setThumbnailAccessory(
                        new ThumbnailBuilder()
                            .setURL("https://i.imgur.com/thdXp8V.png")
                    )
                    .addTextDisplayComponents(
            ...
            .addTextDisplayComponents(
                new TextDisplayBuilder().setContent(`${userdb.personagens.map(p => `**${p.nome}** | Constelação: ${c = p.c}`).join(" \n ")}`),
            )
            .addSeparatorComponents(
                new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true),
            ),]
